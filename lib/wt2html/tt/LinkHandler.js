...
WikiLinkHandler.prototype.handleInfo = function(cb, token, opts, optSources, errs, info, manualinfo) {
...
WikiLinkHandler.prototype.renderFile = function(token, frame, cb, target) {
	...
	if (!env.conf.parsoid.fetchImageInfo) {
		...
		return this.handleInfo(cb, token, opts, optSources, [err], errorInfo(opts));
	}
	...
	if (opts.manualthumb) {
		...
		reqs.push(
			env.batcher.imageinfo(manualThumbTitle.getKey(), opts.size.v)
	...
	.then(function(result) {
		...
		if (!info) {
			info = errorInfo(opts);
			errs.push(makeErr('apierror-filedoesnotexist', 'This image does not exist.'));
		...
		if (opts.manualthumb !== undefined) {
			...
			if (!manualinfo) {
				...
				errs.push(makeErr('apierror-unknownerror', manualinfo.thumberror));
			}
		}
		return [errs, info, manualinfo];
	}, function(e) {
		err = makeErr('apierror-unknownerror', e);
		return [[err], errorInfo(opts)];
	})
	.spread(this.handleInfo.bind(this, cb, token, opts, optSources))
	.done();
};
